...
#if FTS_CUST_PM
static int fts_cust_power_event(struct notifier_block *nb,
			       unsigned long action, void *data)
{
    ...
    switch(action) {
        case PM_POST_HIBERNATION:
            //FTS_INFO("---PM_POST_HIBERNATION---");
            break;
        case PM_POST_SUSPEND:
            //FTS_INFO("---PM_POST_SUSPEND---");
            queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
            break;
        case PM_HIBERNATION_PREPARE:
            //FTS_INFO("---PM_HIBERNATION_PREPARE---");
            break;
        case PM_SUSPEND_PREPARE:
            //FTS_INFO("---PM_SUSPEND_PREPARE---");
...
static struct notifier_block fts_cust_power_notifier = {
    .notifier_call = fts_cust_power_event,
};
#endif
...
#endif
#if FTS_CUST_PM
	register_pm_notifier(&fts_cust_power_notifier);
#endif
...
#if FTS_CUST_PM
	unregister_pm_notifier(&fts_cust_power_notifier);
#endif
